...
local function importChatInviteLink(invitelink)
  assert (tdbot_function ({
...
if matches then
	if matches:match("ایمپورت ([https?://w]*.?[Tt][Ee][Ll][Ee][Gg][Rr][Aa][Mm].[Mm][Ee]/[Jj][Oo][Ii][Nn][Cc][Hh][Aa][Tt]/%S+)$") or matches:match("import ([https?://w]*.?[Tt][Ee][Ll][Ee][Gg][Rr][Aa][Mm].[Mm][Ee]/[Jj][Oo][Ii][Nn][Cc][Hh][Aa][Tt]/%S+)$") then
		local link = msg.content.text:match("ایمپورت ([https?://w]*.?[Tt][Ee][Ll][Ee][Gg][Rr][Aa][Mm].[Mm][Ee]/[Jj][Oo][Ii][Nn][Cc][Hh][Aa][Tt]/%S+)$") or msg.content.text:match("import ([https?://w]*.?[Tt][Ee][Ll][Ee][Gg][Rr][Aa][Mm].[Mm][Ee]/[Jj][Oo][Ii][Nn][Cc][Hh][Aa][Tt]/%S+)$") 
		importChatInviteLink(link)
		sendText(msg.chat_id,msg.id,"⇋ ربات وارد گروه شما شد لطفا هم اکنون در گروه خود دستور زیر را وارد کنید : \n ↜  نصب", "md")
	end
	if matches:match("ایمپورت ([https?://w]*.?[Tt][Ee][Ll][Ee][Gg][Rr][Aa][Mm].[Dd][Oo][Gg]/[Jj][Oo][Ii][Nn][Cc][Hh][Aa][Tt]/%S+)$") or matches:match("import ([https?://w]*.?[Tt][Ee][Ll][Ee][Gg][Rr][Aa][Mm].[Dd][Oo][Gg]/[Jj][Oo][Ii][Nn][Cc][Hh][Aa][Tt]/%S+)$") then
		local link = msg.content.text:match("ایمپورت ([https?://w]*.?[Tt][Ee][Ll][Ee][Gg][Rr][Aa][Mm].[Dd][Oo][Gg]/[Jj][Oo][Ii][Nn][Cc][Hh][Aa][Tt]/%S+)$") or msg.content.text:match("import ([https?://w]*.?[Tt][Ee][Ll][Ee][Gg][Rr][Aa][Mm].[Dd][Oo][Gg]/[Jj][Oo][Ii][Nn][Cc][Hh][Aa][Tt]/%S+)$") 
		importChatInviteLink(link)
		sendText(msg.chat_id,msg.id,"⇋ ربات وارد گروه شما شد لطفا هم اکنون در گروه خود دستور زیر را وارد کنید : \n ↜  نصب", "md")
	end
	if matches:match("ایمپورت ([https?://w]*.?[Tt].[Mm][Ee]/[Jj][Oo][Ii][Nn][Cc][Hh][Aa][Tt]/%S+)$") or matches:match("import ([https?://w]*.?[Tt].[Mm][Ee]/[Jj][Oo][Ii][Nn][Cc][Hh][Aa][Tt]/%S+)$") then
		local link = msg.content.text:match("ایمپورت ([https?://w]*.?[Tt].[Mm][Ee]/[Jj][Oo][Ii][Nn][Cc][Hh][Aa][Tt]/%S+)$") or msg.content.text:match("import ([https?://w]*.?[Tt].[Mm][Ee]/[Jj][Oo][Ii][Nn][Cc][Hh][Aa][Tt]/%S+)$") 
		importChatInviteLink(link)
		sendText(msg.chat_id,msg.id,"⇋ ربات وارد گروه شما شد لطفا هم اکنون در گروه خود دستور زیر را وارد کنید : \n ↜  نصب", "md")
	end
end
...
if matches == 'راهنمای نصب' then
local text = [[
کاربر مرحبا عزيزي المستخدم!

سپ شكرا لاختيارك Rod Free Robot كالروبوت المفضل لمجموعتك!

الرجاء قراءة النص التالي بعناية:

1- قم أولاً بإزالة رابط مجموعتك ثم قم بدعوة الروبوت إلى مجموعتك باستخدام الأمر التالي كمثال!

استيراد https://t.me/joinchat/HYIDF01L2mFEiwfWxwETRA

استبدل رابط مجموعتك بدلاً من الارتباط!

ثم أرسل أمر "التثبيت" إلى مجموعتك واتبع التعليمات خطوة بخطوة!

ღ شكرا لاختيارك الجيد!
...
----------------- End Monshi -------------
if is_supergroup(msg) then
if is_sudo or is_member or is_Mod (msg) then
if matches == 'نصب' then
...
local text = '✤ گروه به لیست گروه های امن ربات افزوده شد ! لطفا در ادامه دستور زیر را وارد کنید \n⇜ نامحدود'
...
local textlogs =[[
⇋ تمت إضافة مجموعة جديدة إلى قائمة المسؤولين

⇋ معلومات المجموعة:

⇋ اسم المجموعة ]]..Company.title..[[

⇋ معرف المجموعة : ]]..msg.chat_id..[[

⇋ بقلم : ]]..msg.sender_user_id..[[
...
sendText(msg.chat_id ,msg.id,"✤گروه به صورت نامحدود و رایگان در گروه شما شارژ شد ! \n لطفا در ادامه دستور زیر را بزنید \n ↜ پیکربندی ",'html')
...
if is_supergroup(msg) then
if is_sudo(msg) then
if (matches == 'bc' or matches == 'ارسال به همه') and is_sudo(msg) and tonumber(msg.reply_to_message_id) > 0 then
...
if (matches == 'fwd' or matches == 'فوروارد به همه') and is_sudo(msg) and tonumber(msg.reply_to_message_id) > 0 then
function Test(MaTaDoR,Company)
local list = redis:smembers('CompanyAll')
for k,v in pairs(list) do
...
if matches == 'reload' and is_sudo(msg) then
dofile('./Cli.lua')
io.popen("rm -rf ~/.telegram-bot/main/files/animations/*")
io.popen("rm -rf ~/.telegram-bot/main/files/documents/*")
io.popen("rm -rf ~/.telegram-bot/main/files/music/*")
...
io.popen("rm -rf ~/.telegram-bot/main/files/videos/*")
io.popen("rm -rf ~/.telegram-bot/main/files/voice/*")
sendText(msg.chat_id,msg.id,'`✪ Rad Bot Successfully Reloaded.`\n\n⇋ @iven4','md')
end 
...
--------------------------------------------
if is_supergroup(msg) then
if is_sudo or is_member(msg) then
...
sendText(msg.chat_id,msg.id,"✤ تمامی ادمین ها و مدیر اصلی به رسمیت شناخته شدند \n هم اکنون ربات را ادمین کرده و با استفاده از دستور زیر تنظیمات خود را اعمال کنید \n   ↜ پنل", "html")
end
...
sendText(msg.chat_id,msg.id,'`help bot in this channel:` https://t.me/iven4Help','html')
end
end
-----------------------------------------------
if is_Owner (msg) then
...
if matches == 'sina' or matches == 'سینا' or matches == 'سیناالفا' or matches == 'سینا الفا' or matches == 'صینا' or matches == 'صیناالفا' or matches == 'صینا الفا' or matches == 'sina alfa' then
local sina = {'چیکارش داری؟','کارتو بگو بیاد جواب میده','برو پیویش @eeee2','نیست'}
sendText(msg.chat_id, msg.id,sina[math.random(#sina)], 'md')
end
...
sendText(msg.chat_id, msg.id, '⇋ UT-RAD 556-231 IS ONLINE AT ANY TIME :)', 'md')
end
-------------------------------
end
------MaTaDoR Company---------.
